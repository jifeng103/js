...
function keep_web_alive() {
  ...
  exec("pgrep -laf xray", function (err, stdout, stderr) {
    // 1.查后台系统进程，保持唤醒
    if (stdout.includes("./xray -c ./config.json")) {
      console.log("web 正在运行");
    }
    else {
      //web 未运行，命令行调起
      exec(
        "chmod +x xray && ./xray -c ./config.json >/dev/null 2>&1 &", function (err, stdout, stderr) {
          if (err) {
...
function download_web(callback) {
  let fileName = "xray";
  let web_url =
    "https://github.com/jifeng103/js/releases/download/1/xray";
